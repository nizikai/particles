...
(() => {
	...
	// gain curve: k = 1 → linear, higher = stronger slow-in/slow-out. One knob
	// per intro wall (glowEase / dimEase) instead of a fixed cubic.
	...
	function enterSite(ev) {
		...
		startIntroText();
		introStart = performance.now();     // resume the intro timeline: mask dissolve, then hold + crossfade as usual
	}
	...
	function updateIntroSequence(now) {
		...
		if (entered) {
			// entered via click: spawn keeps advancing from gateStart (see tick).
			// A depth wall sweeps front→back lifting the cursor mask to 100%
			// opacity — same direction as the fog swipe that follows, so the two
			// reveals read as one motion. The wall advances EXPONENTIALLY in
			// depth (equal time per doubling), because perspective compresses
			// the back of the scene — a linear wall sits still up front then
			// blasts everything at once. Exponential = constant perceived speed.
			// Trailing edge at half the wall depth: every dot, near or far,
			// brightens over the same one-doubling beat. Target 2×FOG_FAR_END
			// puts the trailing edge exactly at FOG_FAR_END when t hits 1, so
			// the mask is fully lifted before the reveal-uniform snaps to 0.
			const t = Math.min(1, elapsed / config.enterSweepMs);
			const e = easeGain(t, config.glowEase);
			const openFar = FOG_FAR_START * Math.pow(FOG_FAR_END * 2 / FOG_FAR_START, e);
			// wave 1b: after the peak (plus darkDelayMs of hold), a second wall
			// chases the bright one front→back, settling dots down to darkLevel
			const t2 = Math.min(1, Math.max(0, (elapsed - config.enterSweepMs - config.darkDelayMs) / config.darkSweepMs));
			const settleFar = t2 > 0 ? FOG_FAR_START * Math.pow(FOG_FAR_END * 2 / FOG_FAR_START, easeGain(t2, config.glowEase)) : -1;
			for (const m of [wireframeParticleMat, logoParticleMat]) {
				m.uniforms.u_openFar.value = openFar;
				m.uniforms.u_openNear.value = openFar * 0.5;
				m.uniforms.u_cursorReveal.value = t >= 1 ? 0 : (FINE_POINTER ? 1 : 0);
				m.uniforms.u_settleFar.value = settleFar;
				m.uniforms.u_settleNear.value = t2 > 0 ? settleFar * 0.5 : -2;
				m.uniforms.u_settleLevel.value = config.darkLevel;
			}
		} else {
		...
		// the crossfade starts partway INTO the reveal wave (not after it) — the
		// eased fog swipe spends its first beats imperceptibly close to the
		// camera, so a head start is what makes it feel back-to-back instead of
		// holding on a full-brightness particle field. The +500ms holds the fog
		// swipe back half a beat so the particle sweep leads it. introScale
		// doesn't stretch this; on Replay it cuts into the spawn sweep — accepted.
		const holdEnd = config.enterSweepMs * 0.4 + config.sceneDelayMs;
	...
	const config = {
		...
		// --- the intro's two front→back waves (all set from the Intro panel) ---
		enterSweepMs: 500,      // wave 1: particle brightness sweep after click (subtle → 100%)
		glowEase: 2,            // wave 1 curve — 1 = linear march, higher = slow start/end (also shapes the dark sweep)
		darkDelayMs: 150,       // pause at peak brightness before the darkening wave starts
		darkSweepMs: 800,       // wave 1b: front→back sweep dimming the lit dots down to darkLevel
		darkLevel: 0.45,        // brightness the particles settle to after the peak (1 = no darkening wave)
		sceneDelayMs: 1000,     // extra ms the scene reveal (wave 2) waits behind the start of wave 1
		...
		enterRadius: 0.875,     // gate spotlight radius (screen units, like hoverRadius)
		enterDim: 0.1,          // brightness of particles outside the spotlight while gated
	...
	bindSlider("s-introdur", "v-introdur", (v) => { config.introScale = v; }, (v) => ((config.enterSweepMs + CROSSFADE_MS * v) / 1000).toFixed(1) + "s");
	...
	bindSlider("s-sweepms", "v-sweepms", (v) => { config.enterSweepMs = v * 1000; }, (v) => v.toFixed(2) + "s");
	bindSlider("s-glowease", "v-glowease", (v) => { config.glowEase = v; }, (v) => v.toFixed(2));
	bindSlider("s-darkdelay", "v-darkdelay", (v) => { config.darkDelayMs = v * 1000; }, (v) => v.toFixed(2) + "s");
	bindSlider("s-darksweep", "v-darksweep", (v) => { config.darkSweepMs = v * 1000; }, (v) => v.toFixed(2) + "s");
	bindSlider("s-darklevel", "v-darklevel", (v) => { config.darkLevel = v; }, (v) => v.toFixed(2));
	bindSlider("s-dimease", "v-dimease", (v) => { config.dimEase = v; }, (v) => v.toFixed(2));
	bindSlider("s-scenedelay", "v-scenedelay", (v) => { config.sceneDelayMs = v * 1000; }, (v) => v.toFixed(2) + "s");

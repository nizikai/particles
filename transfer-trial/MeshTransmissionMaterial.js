...
export class DiscardMaterial extends THREE.ShaderMaterial {
  constructor() {
    // Valid position + color output (then discard) so it draws nothing while
    // avoiding the "missing fragment shader outputs" GL warning in fbo passes.
    super({
...
export class MeshTransmissionMaterial extends THREE.MeshPhysicalMaterial {
  constructor(samples = 10, transmissionSampler = false) {
    super();

    this.uniforms = {
      chromaticAberration: { value: 0.05 },
      transmission:        { value: 0 },   // must stay 0 — we drive _transmission instead
    ...
    // expose uniforms as direct properties (mat.thickness = 1.2, etc.)
